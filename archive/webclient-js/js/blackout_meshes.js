 ...
 *
 * An ES MODULE. It imports three.js and the glTF loader by name, so the
 * "REQUIRES: three.js, then GLTFLoader.js, both loaded before this file" that
 * used to head this comment is now stated in the imports and enforced by the
 * browser rather than by a script tag someone remembered to put in the right
 * place.
 *
 * Not a plugin. One module that every pane imports, because the answer must
 * not differ between them: the mesh drawn for a spear in the inventory is the
 * mesh drawn for the same spear lying on the ground. Module scope gives that
 * for free -- the model cache below is one cache however many panes import it.
 *
...
import {
ITEM_FAMILY_WEAPON, ITEM_FAMILY_ARMOR, ITEM_FAMILY_JEWELLERY,
ITEM_FAMILY_MATERIAL, ITEM_FAMILY_TOOL, ITEM_FAMILY_CURRENCY,
FAMILY_NPC, FAMILY_CHARACTER, FAMILY_STATION, FAMILY_GATHERABLE
} from "./generated/blackout_constants.js";

// ─── Families ───────────────────────────────────────────────────────────

// Imported from the generated module above rather than read off a global.
// The names are used unaliased below; a typo is now a load-time error that
// names the symbol, instead of an `undefined` that silently sent every item
// of that family to the generic mesh.
const FAMILY_WEAPON   = ITEM_FAMILY_WEAPON;
const FAMILY_ARMOR    = ITEM_FAMILY_ARMOR;
const FAMILY_JEWEL    = ITEM_FAMILY_JEWELLERY;
const FAMILY_MATERIAL = ITEM_FAMILY_MATERIAL;
const FAMILY_TOOL     = ITEM_FAMILY_TOOL;
const FAMILY_CURRENCY = ITEM_FAMILY_CURRENCY;
...
const loader = function () {
    ...
    // No "is the loader there?" check any more. It is an IMPORT: if it
    // could not be fetched, this module never evaluated and nothing below
    // ran. The old guard existed because the loader arrived as a global
    // that a misordered script tag could leave undefined, and its
    // fallback -- render everything procedurally, warn once -- silently
    // degraded every model in the game to a grey blob.
...
export { resolve, release, registerModel, hasModel };
