...
import {
    ...
    FAMILY_CHARACTER, KIND_STEP, KIND_WALK, KIND_LOOK, KIND_CANCEL,
    SUBSCRIBE_ALL, TILE_KEY_TEMPLATE
} from "../generated/blackout_constants.js";

const blackout3d = (function () {

// ─── Server vocabulary ──────────────────────────────────────────────────

// Every name below is IMPORTED from generated/blackout_constants.js at the
// top of this file. They were twelve string literals, then reads off a
// `window.blackoutConstants` global; as imports, a name the generated
// module does not export is a load-time error that says which one, rather
// than an `undefined` channel this pane would quietly subscribe to.
...
// FAMILY_CHARACTER is the one family name this pane asks for on its own
// behalf rather than passing through from a payload: the local player's
// mesh. Imported like the rest.
...
// There is deliberately NO grid-delta -> direction-name table here any
// more, for the same reason there is no entity verb table: the server
// names the whole command. It read +y as north because tileWorldPos maps
// +y to -Z, which made it a fact about this pane's camera pretending to be
// a fact about the world — and it could not survive a map whose geometry
// and directions disagree, or a one-way exit. serializers.tile_actions
// reads the mapping off the room's real spawned exits instead.
...
// No `goto` here either, and no `look`. Both used to be spelled out and
// assembled in tileAction; the server now sends them whole, on the map
// node (`goto (X,Y)`) and on room_info (`look`, and bare `goto` to abort).
//
// Everything about a walk belongs to the server: Dijkstra over the
// contrib's baked path matrix, `interrupt_path` map nodes that stop it,
// and a re-path when the player walks off-route by hand. None of that was
// ever worth reimplementing here, and a second pathfinder would be a
// second answer to "what is the shortest way there".
//
...
//
// A key here must be a room kind some map actually declares. It is not
// checked by anything this file can reach, so it is checked from the side
// that owns the fact — see ClientRoomKindTests in
// systems/statefeed/tests/test_client_constants.py. That test exists
// because this table named "Pole clearing" for months and no map has ever
// declared it: both real clearings rendered the hash colour, nothing
// errored, and the dead key had already been copied into the Godot client.
const ROOM_KIND_COLORS = {
    ...
    // oasis grows rusty poles, oasis_outskirts grows metal ones. Coloured
    // for the material rather than for the tile, so the map reads as a
    // gradient from scrap to stock as the player moves out.
    "Rusty pole clearing":        0xcc6633,
    "Metal pole clearing":        0x8899a6,
    "Shopkeeper":                 0xddcc44,
...
// The key a SERVER tile-action map is looked up by, and the key this pane
// tracks a walk target with. Both must agree with what the server built,
// and the format is the server's — see const.TILE_KEY_TEMPLATE.
//
// Three places used to spell this out as `x + ":" + y`. A change to the
// format on the Python side would have made every lookup miss, silently:
// no entry found means "fall through to the node's goto", so walls would
// have quietly become walkable rather than anything erroring.
const actionKey = function (x, y) {
    return TILE_KEY_TEMPLATE
        .replace("{x}", x)
        .replace("{y}", y);
};

// A different key, for a different job: this one identifies a MESH in this
// pane's own caches and spans every map, so it carries z. It is internal
// and nothing on the wire uses it.
...
const drawMap = function (z) {
    ...
    data.nodes.forEach(function (node) {
        ...
        // `action` is the server's: the pathfinder walk to this node,
        // stamped on the map once per session because it does not change
        // with where the player stands. tileAction picks between it and
        // the nearer, per-move actions on room_info.
        mesh.userData.tile = {
            z: z, x: node.x, y: node.y, action: node.action || null
        };
        tileGroup.add(mesh);
        tileMeshes[key] = mesh;
...
// Two lookups and one piece of client state. There are deliberately no
// rules about the map left in here.
//
// WHAT THIS USED TO BE: a chain that decided for itself that a different z
// is unreachable, that a cardinal neighbour with no exit is a wall, that a
...
const tileAction = function (tile) {
    ...
    // The one comparison that stays. A tile's own `action` is a `goto`,
    // and `goto` does not cross maps — the islands are joined by
    // transition NODES you walk onto, not by coordinates that relate. This
    // is "which island is this", not a rule about walking.
    if (tile.z !== here[2]) {
        return null;
    }
    const key = actionKey(tile.x, tile.y);
    const near = (currentRoom.tile_actions || {})[key];
    ...
    if (near) {
        ...
        // Standing here, mid-walk: the click means stop. The server sent
        // both commands; which one applies is the client's own tracking,
        // because the client is what started the walk.
        if (autoWalkTarget && near.kind === KIND_LOOK &&
                currentRoom.cancel_action) {
            return currentRoom.cancel_action;
        }
        return near;
    }
...
// `tile` is passed so a walk can be tracked to the tile that was CLICKED.
// The server names the command and what it does to a walk; where the walk
// is going is the one part the client already knows, because it is the
// thing under the cursor.
const runAction = function (action, tile) {
    if (!action) {
        return;
    }
    sendCommand(action.command);

    if (action.kind === KIND_WALK) {
        autoWalkTarget = tile ? actionKey(tile.x, tile.y) : null;
    } else if (action.kind === KIND_STEP || action.kind === KIND_CANCEL) {
...
const noteArrival = function (data) {
    ...
    if (actionKey(data.coords[0], data.coords[1]) === autoWalkTarget) {
        autoWalkTarget = null;
    }
};
...
const onPickPointerUp = function (event) {
    ...
    runAction(picked.action, picked.mesh && picked.mesh.userData
        ? picked.mesh.userData.tile : null);
};
...
const buildPane = function (glContainer) {
    ...
    // The three "is my dependency loaded?" guards that used to stand here
    // -- for THREE, blackoutMeshes and the generated constants -- are gone.
    // All three are now imports: if any could not be fetched, this module
    // never evaluated and this pane was never registered, so there is no
    // state in which buildPane runs without them. The guards existed
    // because those arrived as globals a misordered script tag could leave
    // undefined.
...
const subscribe = function () {
    Evennia.msg("blackout_subscribe", [], { channels: SUBSCRIBE_ALL });
    log("subscribe sent");
};
...
return {
    ...
    // Exposed for testing, and it is the ONE routine in this file worth
    // exposing for it: every click a player makes goes through it, and the
    // rules it used to hold were wrong once already in a way no screenshot
    // showed. It is pure — a tile in, an action or null out, reading only
    // the feed state this module has recorded — so it can be exercised
    // from node with no DOM, no three.js and no browser.
    //
    // See the tileAction cases in the client smoke tests. `getSummary`
    // above set the precedent for a read-only diagnostic on this object;
    // plugin_handler ignores keys it does not know.
    getTileAction: function (tile) { return tileAction(tile); }
};
})();

// plugin_handler is a CLASSIC global, defined by Evennia's webclient_gui.js,
