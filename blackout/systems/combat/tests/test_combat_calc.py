...
Description: Pure-function pytest suite for OSRS-exact math in systems/combat/combat_calc.py.
...
def test_effective_level_no_bonuses_10():
    assert effective_level(10) == 18


def test_effective_level_max_with_stance():
    assert effective_level(127, stance_bonus=3) == 138


def test_effective_level_with_set_multiplier():
...
def test_attack_roll_no_equip():
    assert attack_roll(110, 0) == 110 * 64


def test_defense_roll_equalizes_attack():
...
def test_hit_chance_equal_rolls_approx_half():
    r = attack_roll(10, 0)
    p = hit_chance(r, r)
    assert 0.49 < p < 0.50


def test_hit_chance_double_attacker_greater():
    r = attack_roll(10, 0)
    p = hit_chance(2 * r, r)
    assert 0.74 < p < 0.76


def test_defender_equip_bonus_lowers_hit_chance():
...
def test_resolve_swing_miss_returns_zero():
...
def test_resolve_swing_hit_produces_damage():
    rng = random.Random(1)
