...
Why this is not in tick_engine
------------------------------
The engine's job is to advance handlers. Deciding who is watching, and how a
tick reads, is a different fact with a different lifetime. tick_engine gains
...
def _find_engine():
    ...
    from .tick_engine import TICK_ENGINE_KEY
...
def _capture_combat(engine, character) -> _CombatSnapshot:
    ...
    tick_engine._tickable_handler_keys: the handler modules and the engine
    reference each other, and resolving that at import time closes the cycle.
    """
    from .combat import get_handler_for
...
def _capture_aura(engine, character) -> _AuraSnapshot:
    ...
    from .auras.aura_handler import get_aura_handler_for
...
def _late_threshold() -> float:
    ...
    threshold = const.COMBAT_TICK_SECONDS * const.TICK_DEBUG_LATE_TICK_FACTOR
...
def _interval_stats(engine) -> dict:
    ...
    Module Globals:
        const.COMBAT_TICK_SECONDS and const.TICK_DEBUG_LATE_TICK_FACTOR read.
...
def _snapshot_engine(engine) -> list:
    ...
    lines = [
        ...
        f" at {const.COMBAT_TICK_SECONDS}s nominal",
