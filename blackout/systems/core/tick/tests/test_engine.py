...
from systems.core.tick.engine import TICK_ENGINE_KEY, BlackoutTickEngine, get_tick_engine
...
class TestStaleTypeclassPath(EvenniaTestCase):
    ...
    def _break_the_stored_path(self):
        ...
        ScriptDB.objects.filter(id=engine.id).update(
            db_typeclass_path="systems.gameplay.combat.tick_engine.BlackoutTickEngine"
        )
