...
from systems.core.tick import tickable
from systems.gameplay.combat.auras.aura_handler import BlackoutAuraHandler
from systems.gameplay.combat.combat import BlackoutCombatHandler


# Public constant definitions

# Where to look for TickableHandler subclasses -- the whole systems/ tree,
# three levels up from systems/core/tick/tests/. Scanned as TEXT, never
...
class TestManagerRegistry(EvenniaTestCase):
    ...
    def test_every_known_manager_is_registered(self):
        from systems.core import managers

        managers.load_all_managers()
        names = set(managers.MANAGER_REGISTRY)

        self.assertIn("systems.core.tick.engine.bootstrap_tick", names)
        self.assertIn("systems.gameplay.spawning.respawn.bootstrap_respawns", names)
        self.assertIn("systems.gameplay.combat.hp_regen.bootstrap_regen", names)
    ...
    def test_one_failing_manager_does_not_stop_the_others(self):
        ...
        self.assertIsNotNone(started["systems.core.tick.engine.bootstrap_tick"])
...
class TestPurgeClearsAccessorCaches(EvenniaTest):
    ...
    def test_purging_clears_the_cached_handler(self):
        from systems.core.tick.engine import purge_stale_handlers
