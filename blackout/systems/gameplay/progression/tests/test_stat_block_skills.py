...
from systems.gameplay.progression.skills.stat_block import SKILL_LEVELS_ATTR, StatBlockSkills
...
class TestStatBlockSkillReads(EvenniaTest):
    ...
    def setUp(self):
        ...
        self.skills.seed({
            skill_constants.SKILL_KEY_STRIKE: 20,
            skill_constants.SKILL_KEY_BRAWN: 15,
            skill_constants.SKILL_KEY_DEFENSE: 10,
            skill_constants.SKILL_KEY_FORTITUDE: 50,
        })

    def test_a_declared_level_reads_back(self):
        self.assertEqual(self.skills.get_level(skill_constants.SKILL_KEY_STRIKE), 20)
        self.assertEqual(self.skills.get_level(skill_constants.SKILL_KEY_FORTITUDE), 50)
    ...
    def test_an_undeclared_skill_reads_the_absent_floor(self):
        ...
        in this" means."""
        self.assertEqual(
            self.skills.get_level(skill_constants.SKILL_KEY_GUNS),
            skill_constants.DEFAULT_START_LEVEL,
        )
    ...
    def test_reading_an_undeclared_skill_does_not_write_one(self):
        ...
        self.assertNotIn(skill_constants.SKILL_KEY_GUNS, stored)
    ...
    def test_reads_survive_a_fresh_handler(self):
        ...
        self.assertEqual(other.get_level(skill_constants.SKILL_KEY_STRIKE), 20)

    def test_an_entity_that_was_never_seeded_reads_the_floor(self):
        virgin = StatBlockSkills(self.obj2)

        self.assertEqual(
            virgin.get_level(skill_constants.SKILL_KEY_STRIKE),
            skill_constants.DEFAULT_START_LEVEL,
        )
...
class TestStatBlockSkillWrites(EvenniaTest):
    ...
    def test_set_level_persists(self):
        self.skills.set_level(skill_constants.SKILL_KEY_STRIKE, 33)

        self.assertEqual(StatBlockSkills(self.obj1).get_level(skill_constants.SKILL_KEY_STRIKE), 33)

    def test_set_level_returns_what_it_stored(self):
        self.assertEqual(self.skills.set_level(skill_constants.SKILL_KEY_STRIKE, 33), 33)

    def test_set_level_clamps_to_the_ceiling(self):
        stored = self.skills.set_level(skill_constants.SKILL_KEY_STRIKE, 9999)

        self.assertEqual(stored, skill_constants.MAX_BASE_SKILL_LEVEL)

    def test_set_level_clamps_to_the_floor(self):
        stored = self.skills.set_level(skill_constants.SKILL_KEY_STRIKE, -5)
    ...
    def test_modify_level_drains(self):
        ...
        remaining = self.skills.modify_level(skill_constants.SKILL_KEY_DEFENSE, -4)

        self.assertEqual(remaining, 6)
        self.assertEqual(self.skills.get_level(skill_constants.SKILL_KEY_DEFENSE), 6)

    def test_modify_level_buffs(self):
        self.assertEqual(self.skills.modify_level(skill_constants.SKILL_KEY_DEFENSE, 7), 17)
    ...
    def test_a_drain_past_zero_floors_rather_than_going_negative(self):
        ...
        self.assertEqual(self.skills.modify_level(skill_constants.SKILL_KEY_DEFENSE, -50), 0)

    def test_modifying_an_undeclared_skill_starts_from_the_floor(self):
        self.assertEqual(self.skills.modify_level(skill_constants.SKILL_KEY_GUNS, 3), 3)

    def test_seed_replaces_rather_than_merges(self):
        self.skills.seed({skill_constants.SKILL_KEY_BRAWN: 5})

        self.assertEqual(self.skills.get_level(skill_constants.SKILL_KEY_BRAWN), 5)
        self.assertEqual(
            self.skills.get_level(skill_constants.SKILL_KEY_STRIKE),
            skill_constants.DEFAULT_START_LEVEL,
        )
    ...
    def test_seed_drops_a_non_integer_level(self):
        ...
        self.skills.seed({
            skill_constants.SKILL_KEY_STRIKE: "twenty",
            skill_constants.SKILL_KEY_BRAWN: 5,
        })
...
class TestNpcDefSeeding(EvenniaTest):
    ...
    def test_fortitude_is_derived_from_max_hp(self):
        ...
        self.assertEqual(npc.skills.get_level(skill_constants.SKILL_KEY_FORTITUDE), 87)
