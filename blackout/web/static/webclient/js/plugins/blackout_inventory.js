...
const blackoutInventory = (function () {

// ─── Server vocabulary ──────────────────────────────────────────────────

// IMPORTED from generated/blackout_constants.js -- see the top of this
// file. CH_CHAR_ITEMS was a string literal under a comment asking someone
// to keep it in step with systems/statefeed/constants.py, and it is the
// likeliest of all these to be got wrong by hand: the wire name is
// `char_items_list` while everything Blackout owns says inventory.
...
const dropAction = function (sourceKey, targetKey) {
    ...
    if (fromInventory && frame.kind === "inventory") {
        ...
        // The one command this pane composes rather than reads off the
        // payload, and the only one it can: `swap` takes two endpoints and
        // only the drag knows both, so pre-naming it per item would be
        // every pair of slots. The SPELLING is still the server's -- see
        // const.INVENTORY_SWAP_TEMPLATE -- so the verb and the argument
        // order have one owner even though the gesture does not.
        return INVENTORY_SWAP_TEMPLATE
            .replace("{source}", from)
            .replace("{target}", to);
    }
...
const buildPane = function (glContainer) {
    ...
    // No THREE / blackoutMeshes guards here any more: both are imports,
    // so a fetch failure means this module never evaluated and this pane
    // was never registered. See the same note in blackout3d.js.
...
// plugin_handler is a CLASSIC global from Evennia's webclient_gui.js -- there
// is nothing to import it from. See the matching note in blackout3d.js.
